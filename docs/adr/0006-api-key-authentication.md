...
Reloading is therefore real, but only from sources the process can observe changing. A file the platform updates in place does, including a ConfigMap or Secret mounted as one. Environment variables are read at startup and cannot change, and neither can a file baked into an image, which is the case worth stating because it looks like a file and behaves like a constant. A deployment that wants to retune a client without restarting has to mount that section as a file the platform rewrites in place. Locally the equivalent is to copy the settings file into the running container, which is preferred over bind mounting it because a single file mount binds to the file rather than to its name, and an editor that saves by renaming a new file over the old one leaves the reader attached to something nothing writes to again.

A value that fails validation after a reload makes requests fail with the setting named in the log, and the service recovers once the value is corrected. Failing loudly is the right end of that trade: the alternative is carrying on with numbers nobody can see.
